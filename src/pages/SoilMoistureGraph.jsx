...
import { soilMoistureHourlyValues } from '../constants/soilMoistureHourlyValues';
...
const SoilMoistureGraph = () => {
	...
	const [filteredSoilMoistureValue, setFilteredSoilMoistureValue] = useState(
		[]
	);
	...
	const { isLoading: isLoadingSoilMoisture, data: soilMoistureValue } =
		useQuery(
			'soil-moisture-data',
			() => {
				return axios.get(
	...
	const transformSoilMoistureObject = (data) => {
		const groupedData = {};
		const values = [];
	...
	const handleSubmit = () => {
		if (startDate && endDate) {
			...
			const firstDataDate = new Date(
				soilMoistureValue?.data[0].createdAt.split(' ')[0]
			);
			...
			const filteredSoilMoistureData = filterDataByDateRange(
				soilMoistureValue?.data,
				formattedStartDate,
				formattedEndDate
			);

			setFilteredSoilMoistureValue(filteredSoilMoistureData);
	...
	useEffect(() => {
		if (!isLoadingSoilMoisture) {
			const ctx = chartRef.current.getContext('2d');
			let labels, data;

			if (timePeriod === 'daily') {
				const soilMoistureData = isDateRangeApplied
					? transformSoilMoistureObject(filteredSoilMoistureValue)
					: transformSoilMoistureObject(soilMoistureValue?.data);
				labels = soilMoistureData.formattedDates;
				data = soilMoistureData.values;
			} else if (timePeriod === 'hourly') {
				labels = hourlyLabels;
				data = soilMoistureHourlyValues;
			}
		...
		soilMoistureValue?.data,
		filteredSoilMoistureValue,
		isDateRangeApplied,
		isLoadingSoilMoisture,
		timePeriod
	]);
		...
		<>
			...
			>
				<ToggleButtonGroup
					...
					disabled={isLoadingSoilMoisture}
				>
					<ToggleButton
						value='daily'
						aria-label='daily'
			...
			<div className='chartCard'>
				{!isLoadingSoilMoisture || filterLoading ? (
					<>
