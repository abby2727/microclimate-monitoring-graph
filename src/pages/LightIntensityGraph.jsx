...
const LightIntensityGraph = () => {
	...
	const [filteredLightIntensityValue, setFilteredLightIntensityValue] =
		useState([]);
	...
	const transformLightIntensityObject = (data) => {
		const groupedData = {};
		const values = [];
	...
	const handleSubmit = () => {
		if (startDate && endDate) {
			...
			const firstDataDate = new Date(
				lightIntensityValue?.data[0].createdAt.split(' ')[0]
			);
			...
			const filteredLightIntensityData = filterDataByDateRange(
				lightIntensityValue?.data,
				formattedStartDate,
				formattedEndDate
			);

			setFilteredLightIntensityValue(filteredLightIntensityData);
	...
	return (
		<>
			...
			<div className='chartCard'>
				{!isLoadingLightIntensity || filterLoading ? (
					<>
...
export default LightIntensityGraph;
